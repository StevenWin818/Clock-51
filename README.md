...
为方便用户在需要时取消正在进行或即将进行的报时，程序实现了按键中断机制：

- 在预报时窗口（当分钟为 `59` 且秒为 `55..59`）或整点瞬间（分钟为 `0` 且秒为 `0`）检测到任意按键时，主循环会向蜂鸣器模块请求取消本次整点/预报时的剩余声音。主循环中的实现示例（见 `main.c`）如下：
...
- 蜂鸣器模块提供了非阻塞的取消/抑制接口：
  - `Buzzer_RequestCancelCurrentTop()`：在检测到按键时请求取消本次（当前小时/预报时）剩余的报时。该函数设置一个请求标志，由蜂鸣器状态机在合适时刻处理。
  - `Buzzer_Check()`：由定时或时间变更处调用，用于驱动蜂鸣器状态机并执行取消/抑制逻辑（若存在取消请求，会立即停止正在响的短响/长响并阻止后续的本次整点音）。

- 行为说明：
  - 按任意键即可取消当前正在播放的短响或即将到来的长响，并阻止本次整点（top）继续发声。
  - 该机制是局部的——仅影响当前小时/本次预报时序列，下一小时仍会按照正常规则再次报时。
  - 由于实现为非阻塞模式，取消操作不会阻塞主循环或影响按键/显示逻辑。

如需在其他上下文改变取消逻辑（例如只在特定菜单才允许取消），可以在主循环中添加更细粒度的状态判断后再调用 `Buzzer_RequestCancelCurrentTop()`。
